// stls-warehouse-extend-1.0.7.js
(function () {
  // -------------- small helpers --------------
  ...
  // -------------- parsing helpers --------------
  // normalise label text (e.g., "Return By 17:00 default" -> "Return By")
  function normaliseLabel(lbl){
    if (!lbl) return lbl;
    var t = lbl.trim();
    if (t.toLowerCase().startsWith("return by")) return "Return By";
    if (t.toLowerCase() === "customer/company name") return "Customer/company name";
    return t;
  }

  // turn a raw value into "HH:MM" or "HH:MM - HH:MM" by extracting times only
  function timesOnly(val){
    if (!val) return val;
    var times = (val.match(/\b\d{1,2}:\d{2}\b/g) || []);
    if (!times.length) return val;               // fallback: if no time found, leave as-is
  ...
  function buildInfoBlockFromTooltipNode(tipNode, jobNumberFromTitle){
    ...
    // Only LEAF divs to avoid wrapper text glomming
    ...
    // Inject job number only if needed (we’ll remove it later anyway per your spec)
    ...
    // For Outgoing & Return By: time-only + optional end time (if present)
    ...
    // ---- ORDER: Outgoing, Return By, then the rest in a sensible order ----
    var preferredOrder = [
      "Outgoing",
      "Return By",
